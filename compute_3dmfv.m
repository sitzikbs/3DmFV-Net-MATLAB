function [pc_3dmfv] = compute_3dmfv(points, w, mu, sigma, normalize, flatten)
...
% per gaussian)
% OUTPUT: 
% pc_3dmfv [batch_size x 6*d+2 x n_gaussians]  the 3dmfv representation for each
...
w_per_batch_per_d = repmat(w, [3*D, 1, n_batches]);  % 3D X n_gaussians X D (D for min and D for max and D for sum)
                                
p_per_point = (1.0 / (power(2.0 * pi, D / 2.0) .* power(sigma(1), D))) .* exp(-0.5 .* sum(( batch_points - batch_mu).^2 ./ batch_sigma.^2, 1));
w_p = p_per_point.*batch_w;
Q = p_per_point;
Q = w_p ./ repmat(sum(w_p, 2), 1, n_gaussians, 1);
Q_per_d = repmat(p_per_point, [D, 1, 1, 1] ); 

sqrt_w = sqrt(batch_w);
d_pi_all = (Q - batch_w) ./ sqrt_w;
d_pi = [reshape(max(d_pi_all,[], 3), [1, n_gaussians, n_batches]); reshape(sum(d_pi_all,3), [1, n_gaussians, n_batches])];


 d_mu_all = Q_per_d .* (batch_points - batch_mu) ./ batch_sigma;
 d_mu = (1 ./ sqrt(w_per_batch_per_d)) .* [reshape(max(d_mu_all, [], 3), [D, n_gaussians, n_batches]);...
                                                                   reshape(min(d_mu_all, [], 3), [D, n_gaussians, n_batches]);...
                                                                   reshape(sum(d_mu_all, 3), [D, n_gaussians, n_batches])];


d_sig_all = Q_per_d .* (((batch_points - batch_mu).^2 ./ batch_sigma) - 1);
d_sigma = (1 ./ sqrt(2 * w_per_batch_per_d)) .* [reshape(max(d_sig_all, [], 3), [D, n_gaussians, n_batches]);...
                                                                             reshape(min(d_sig_all, [], 3), [D, n_gaussians, n_batches]);...
                                                                             reshape(sum(d_sig_all, 3), [D, n_gaussians, n_batches])];                                                               
...
if normalize
    ...
    % L2 normaliation
    d_pi = d_pi ./ repmat(sqrt(sum(d_pi.^2, 2)),[1, n_gaussians, 1]);
    d_mu = d_mu ./ repmat(sqrt(sum(d_mu.^2, 2)),[1, n_gaussians, 1]);
    d_sigma = d_sigma ./ repmat(sqrt(sum(d_sigma.^2, 2)),[1, n_gaussians, 1]);
end
...
if ~flatten
    n_axis_gaussians = int8(nthroot(n_gaussians,3));
    pc_3dmfv = pc_3dmfv';
    pc_3dmfv = reshape(pc_3dmfv,[n_axis_gaussians, n_axis_gaussians, n_axis_gaussians, 20]);
end
end
